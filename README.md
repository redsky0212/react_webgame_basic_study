...
* 리셋버튼 클릭시 위쪽 색깔부분도 렌더링 된다.
  - 이런경우에는 보통 그냥 넘어가는데 이것도 렌더링을 피해서 최적화 하고자 할때는 컴포넌트로 분리해서 상황에 따라 PureComponent나 memo로 작업한다.
* Hooks컴포넌트는 렌더링 될때 함수 전체가 다시 렌더링 되므로 무거운 공통함수를 콜 하는경우에는 문제가 될 수도있다.
